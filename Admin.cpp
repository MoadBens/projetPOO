...
void Admin::ModifierUtilisateur(std::string nom, std::string prenom, Personnel* p) {
    ...
    //ENVOI DE LA REQUETE
    sqlite3_stmt* stmt;
    std::string sql = "UPDATE Connexion SET NOM='"+p->get_nom() + "',PRENOM='" + p->get_prenom()+"',NIVEAU='" + std::to_string(p->get_niveau()) + 
        "',MDP='"+p->get_mdp() + "'WHERE NOM ='" + nom + "' AND PRENOM='" + prenom + "'";
    rc = sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        std::cerr << "Error preparing SQL statement: " << sqlite3_errmsg(db) << std::endl;
        sqlite3_finalize(stmt);
